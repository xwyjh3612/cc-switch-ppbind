...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.ppbind/skills/ 或 ~/.agents/skills/）
    ...
    /// 获取 Skill 卸载备份目录（~/.ppbind/skill-backups/）
    ...
    fn resolve_uninstall_backup_source_excluding(
        ...
        // 返回值会被整目录复制进 ~/.ppbind/skill-backups/ 并由 get_skill_backups
...
mod tests {
    ...
    fn restore_from_backup_rejects_traversal_directory_in_metadata() {
        ...
        // SSOT 位于 {home}/.ppbind/skills，"../../pwned-restore" 若生效会写到 {home}/pwned-restore。
    ...
    fn uninstall_rejects_traversal_directory_from_db_row() {
        ...
        // 与 import_sql_string_for_sync 的效果一致）。SSOT = {home}/.ppbind/skills，
    ...
    fn migrate_storage_retargets_an_equivalent_relative_pi_symlink() {
        ...
        std::os::unix::fs::symlink(Path::new("../../.ppbind/skills/test-skill"), &pi_skill)
