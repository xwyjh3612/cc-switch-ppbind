...
mod tests {
    ...
    #[test]
    fn parse_session_reads_last_assistant_model() {
        let temp = tempdir().expect("tempdir");
        let path = temp.path().join("session-model.jsonl");
        std::fs::write(
            &path,
            concat!(
                "{\"sessionId\":\"session-model\",\"cwd\":\"/tmp/project\",\"timestamp\":\"2026-03-06T10:00:00Z\"}\n",
                "{\"type\":\"assistant\",\"message\":{\"role\":\"assistant\",\"model\":\"claude-sonnet-4-6\",\"content\":\"old\"},\"timestamp\":\"2026-03-06T10:01:00Z\"}\n",
                "{\"type\":\"assistant\",\"message\":{\"role\":\"assistant\",\"model\":\"claude-opus-4-6\",\"content\":\"latest\"},\"timestamp\":\"2026-03-06T10:02:00Z\"}\n"
            ),
        )
        .expect("write session");

        let meta = parse_session(&path).unwrap();
        assert_eq!(meta.last_model.as_deref(), Some("claude-opus-4-6"));
    }
