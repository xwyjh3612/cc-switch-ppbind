...
impl ProviderRouter {
    ...
    /// Select one explicitly requested provider for a project-scoped route.
    ///
    /// Project overrides intentionally bypass the global failover queue. The
    /// caller still uses the normal circuit breaker at request execution time.
    pub async fn select_specific_provider(
        &self,
        app_type: &str,
        provider_id: &str,
