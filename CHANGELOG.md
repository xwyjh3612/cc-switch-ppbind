...
- Anonymous Codex helper threads can now inherit the latest valid effective route for their requested model within a 10-minute window. The cache keeps up to five distinct routes per model and always prefers the newest still-valid channel, prioritizing successful helper requests when no stronger session or project signal is available.
- Backend tests now use platform-appropriate temporary paths and read the configured proxy listen port instead of hardcoding the legacy 15722 value, so the Windows, macOS and Linux CI jobs pass consistently.
